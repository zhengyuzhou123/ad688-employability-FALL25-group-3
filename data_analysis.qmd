...
```{python}
# --- 4.0 Setup & Load (robust, fixed indent & order) ---
...
# Quick ping to prove Python chunk executes
...
# Check dataset exists BEFORE reading
...
df = pd.read_csv(DATA_PATH, low_memory=False, on_bad_lines="skip")
    ...
    log(f"Columns (head): {list(df.columns)[:12]} ...")
