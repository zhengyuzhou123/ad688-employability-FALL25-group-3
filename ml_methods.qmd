...
# 等价于 pyspark.trim(col("REMOTE_TYPE_NAME"))
df_analysis["REMOTE_TYPE_NAME"] = df_analysis["REMOTE_TYPE_NAME"].astype(str).str.strip()

# 注意：原始数据中有 [None] 这种字符串，PySpark 那里其实是 null，
# 这里我们也当成 "None" 后按 Onsite 处理
...
# ============================================================
# 3.5 清洗 EDUCATION_LEVELS_NAME -> EDUCATION_LEVELS_CLEAN
# ============================================================
# 等价于 regexp_replace + trim
...
# 1. 生成 label（等价于 StringIndexer on REMOTE_GROUP）
# 如果以后还会有 Hybrid，可以不手动 map，用 LabelEncoder 更保险
label_encoder = LabelEncoder()
df_binary["label"] = label_encoder.fit_transform(df_binary["REMOTE_GROUP"])

# 看一下类别顺序，对应 Spark 里的 index 映射
print("Label mapping:", dict(zip(label_encoder.classes_,
                                 label_encoder.transform(label_encoder.classes_))))

# 2. 生成 features（等价于 indexer + onehot + VectorAssembler）
X = df_binary[categorical_cols + numeric_cols]
y = df_binary["label"]

# ColumnTransformer = “分类变量做 OneHot，数值直接透传”
...
# 如果是稀疏矩阵（scipy.sparse），先转成 dense，方便塞进 DataFrame 一列里
if hasattr(X_features, "toarray"):
    X_dense = X_features.toarray()
else:
    X_dense = np.asarray(X_features)

# 组一个和 Spark 类似的 df_prepared：REMOTE_GROUP | label | features
df_prepared = pd.DataFrame({
    "REMOTE_GROUP": df_binary["REMOTE_GROUP"].reset_index(drop=True),
    "label": y.reset_index(drop=True),     # 就是 df_binary["label"]
    "features": list(X_dense)             # 每一行一个 numpy array
})
...
# df_prepared 里有 REMOTE_GROUP | label | features 三列
# 这里先按行切 70/30，保持整体结构（跟 Spark 一样）

train_data, test_data = train_test_split(
    df_prepared,
    test_size=0.3,        # 30% 做 test，等价 [0.7, 0.3]
    random_state=42,      # 等价 seed=42
)
...
# 假设 train_data / test_data 是 pandas DataFrame，包含 columns: label, features
# features 列里是 numpy array（前面我们已经这样构造过）
...
# 2. Logistic Regression（对应 Spark 的 maxIter=10，可以给个大一点保证收敛）
lr_model = LogisticRegression(
    max_iter=1000,      # Spark 那个 10 次迭代很少，这里给足
    n_jobs=-1
)
lr_model.fit(X_train, y_train)

# 3. Random Forest（对应 numTrees=100, seed=42）
...
# ============================
# 先从 test_data 里取出 X_test / y_test
# ============================
# 如果你是用 df_prepared + train_test_split 得到的 train_data / test_data：
X_test = np.stack(test_data["features"].to_numpy())
y_test = test_data["label"].to_numpy()

# ============================
# 预测：等价于 lr_model.transform(test_data)
# ============================
# Logistic Regression
y_pred_lr = lr_model.predict(X_test)

# 用 predict_proba 拿到正类概率，算 AUC（等价 Spark 的 rawPredictionCol + areaUnderROC）
...
# 且 label: 0 = Onsite, 1 = Remote

# 1. 计算 confusion matrix（按 [0, 1] 顺序）
cm_lr = confusion_matrix(y_test, y_pred_lr, labels=[0, 1])
cm_rf = confusion_matrix(y_test, y_pred_rf, labels=[0, 1])
...
# 2. 转成 plotly 需要的 z（list of list）
lr_z = cm_lr.tolist()
rf_z = cm_rf.tolist()
...
# 3. ColumnTransformer = StringIndexer + OneHotEncoder + VectorAssembler 的组合
...
# 转成 dense，方便塞进 DataFrame 一列（如果你想保持 vector 风格）
...
# 5. ONET_NAME -> ONET_label（等价于 StringIndexer on ONET_NAME）
...
# 6. 组 df_cluster：ONET_NAME | ONET_label | features
...
for k in k_values:
    kmeans = KMeans(
        n_clusters=k,
        random_state=42,    # 对应 seed=42
        n_init="auto"       # 或者设成具体数值，比如 10
    )
    # 相当于 Spark 的 model.transform(df_cluster).select("prediction")
    labels = kmeans.fit_predict(X_cluster)

    # 等价于 ClusteringEvaluator(..., metricName='silhouette')
...
cluster_labels = kmeans_final.fit_predict(X_cluster)  # 相当于 transform 得到 prediction/cluster

# 3. 把 cluster 写回 DataFrame
df_clustered = df_cluster.copy()
df_clustered["cluster"] = cluster_labels   # label 0..K-1，等价 Spark 的 cluster 列

# 4. 计算最终 silhouette（等价 evaluator.evaluate(...)）
...
# 5. Cluster 分布（等价 groupBy("cluster").count().orderBy("cluster")）
...
# 如果你想看更像交叉表（行=cluster，列=REMOTE_GROUP）：
...
# 2. 明确建一个只有 "cluster", "count" 两列的 DataFrame
cluster_counts = pd.DataFrame({
    "cluster": cluster_counts_series.index,
    "count": cluster_counts_series.values
})

print(cluster_counts)  # 检查列名是否 ['cluster', 'count']
...
fig = px.bar(
    ...
    color=cluster_counts.values,          # 用数量做颜色映射
    color_continuous_scale="Blues",       # 就是那条蓝色渐变
...
# 1. 先按 STATE_NAME & REMOTE_GROUP 计数（等价 groupBy + count）
state_remote = (
    df_analysis
...
# 2. pivot：行 = STATE_NAME，列 = REMOTE_GROUP（Onsite / Remote / Hybrid）
state_df = (
    state_remote
...
# 3. 确保 Hybrid 这一列存在（防止有的 data 没 Hybrid 就 KeyError）
if "Hybrid" not in state_df.columns:
    state_df["Hybrid"] = 0

# 4. 计算 Total 和 Remote_Pct
...
Figure: Top Positive and Negative Coefficients — This plot highlights the
features that most strongly influence predicted salary levels. Positive
coefficients indicate factors associated with higher predicted salaries,
while negative coefficients indicate factors associated with lower predicted
salaries.

The regression model shows that geographic differences play the largest role.
States like Washington, D.C., California, and New York have strong positive
coefficients, while states such as West Virginia and South Dakota show large
negative coefficients. Remote/hybrid indicators have smaller effects relative
to geography.
...
Figure: Actual vs Predicted Salary — Scatter plot comparing true salary
with model predictions. The dashed line represents perfect prediction (y = x).

The prediction pattern shows that the model captures general salary trends,
but underestimates extremely high salaries and compresses the overall range.
This behavior is consistent with a moderate R² score and suggests that
additional job-related variables (e.g., experience level, seniority) would
