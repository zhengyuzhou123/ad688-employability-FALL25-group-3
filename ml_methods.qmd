...
This graph compares the classification effects of logistic regression and random forest in distinguishing between remote and offline positions.
Both models can identify offline positions very well, but they are more difficult to identify remote positions. Among them, the random forest captures more remote positions, but it also brings more misjudgments.
Overall, industry and regional characteristics have a certain predictive power for whether it is remote, but they are still not sufficient to completely distinguish between the two types of positions.
...
feature_importance_df = feature_importance_df[
    ~feature_importance_df["feature"].str.contains("DURATION", case=False)
]
...
def ellipsize(text, max_len=20):
    return text if len(text) <= max_len else text[:max_len] + "..."

top_n = 10
# Add short names to the complete table
feature_importance_df["feature_short"] = (
    feature_importance_df["feature"]
    .astype(str)
    .apply(lambda x: ellipsize(x, 20))
)
...
This figure shows the results of the random forest feature importance assessment used to distinguish between remote positions and on-site positions in our Lightcast sample (using one-dimensional coded status, NAICS industry, education level and numerical "minimum working years" variables, excluding the factor of working time). The main driving factor of the model is the "minimum working years". In addition, it is also affected by specific states (such as California and Texas) and industry categories, which shows that the availability of remote work is mainly related to the level of work experience and will vary by region/industrys.
...
This map shows the total distribution of jobs in each state.
It can be seen that California and Texas have the highest number of positions, clearly leading. Populous and economically large states such as New York and Florida are also at a relatively high level.
This indicates that job opportunities are highly concentrated in a few states with a concentrated economy and industry, which has significant reference value for job seekers when choosing a working area.
...
This bar chart shows the top 10 states with the highest proportion of remote positions. It can be seen that the proportion of remote work in states such as Alaska, Wyoming and Vermont exceeds 35%, significantly higher than the national average.
This indicates that in these states, enterprises are more inclined to adopt remote models, and it also provides job seekers with more employment opportunities that are not restricted by geography.
